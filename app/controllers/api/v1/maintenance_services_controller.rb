module Api
  module V1
    class MaintenanceServicesController < BaseController
      ...
      before_action :set_maintenance_service, only: %i[ update]

      # GET /api/v1/vehicles/:vehicle_id/maintenance_services
      def index
        services = @vehicle.maintenance_services.includes(:vehicle).references(:vehicle).order(created_at: :desc).page(params[:page]).per(params[:per_page] || 5)
